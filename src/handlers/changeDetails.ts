...
class ChangeDetailsHandler {
    ...
    private questionChannel: TextChannel | undefined;
    ...
    async load() {
        ...
        this.questionChannel = this.guild.channels.cache.get(this.question.channelId as string) as TextChannel;
    ...
    async sendNewQuestionMessage() {
        if (!this.questionChannel || !this.guild) return;
    ...
    async sendSureChangeDetailMessage(dmChannel: DMChannel, detail: string, msg: Message) {
        const sureMsg = await dmChannel.send({ embeds: [Embeds.sureChangeDetail(this.lang)], components: [Components.sureChangeDetailButtons(this.lang)] });
        const buttonCollector = dmChannel.createMessageComponentCollector({ componentType: "BUTTON", time: 10 * 1000 });
        buttonCollector.on('collect', async btn => {
            if (btn.customId === 'cng-dtl-sure') {
                const options: any = {
                    "change-title": async () => this.setTitle(msg.content),
                    "change-description": async () => this.setDescription(msg.content),
                }
